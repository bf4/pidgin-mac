...
//		gfire_close(gc);
	...
	if (bytes_read < packet_len) {
		short_flag = TRUE;
		purple_debug(PURPLE_DEBUG_MISC, "gfire", "Packet read too short, wanted %d bytes, got %d bytes\n",
...
void gfire_parse_packet(PurpleConnection *gc, int packet_len, int packet_id)
{
	...
	switch(packet_id)
	{
		...
		case 137:
			...
			/*_notify_error(gc,"XFire Error","Buddy add Error","User unknown");*/
		break;	
...
void gfire_process_invitation(PurpleConnection *gc, GList *invites)
{
	...
	while (NULL != i){
		...
		buddy->sid = (guint8 *)gc;
		prompt_msg = g_strdup_printf("The user %s wants to add %s to "
				"his or her buddy list%s%s.",
				buddy->name, purple_account_get_username(gc->account),
				buddy->uid_str ? ": " : "",
				buddy->uid_str ? buddy->uid_str : "");
		purple_request_input(gc, NULL, prompt_msg, NULL,
				NULL, FALSE, FALSE, NULL,
				"Authorize", G_CALLBACK(
					gfire_buddy_add_authorize_cb),
				"Deny", G_CALLBACK(
					gfire_buddy_add_deny_cb),
					account, NULL, NULL,
				(void *)buddy);
		g_free(prompt_msg);
		i = g_list_next(i);
